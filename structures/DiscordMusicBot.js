...
class DiscordMusicBot extends Client {
  constructor(props) {
    ...
    if (this.botconfig.Token === "")
      return new TypeError(
        "Le fichier botconfig.js n'est pas rempli. Veuillez vous assurer que rien n'est vide, sinon le bot ne fonctionnera pas correctement.."
      );
    ...
    this.Manager = new Manager({
      ...
      .on("nodeError", (node, error) =>
        this.log(
          `Lavalink: Node ${node.options.identifier} a eu une erreur: ${error.message}`
        )
      )
      .on("trackStart", async (player, track) => {
        this.SongsPlayed++;
        let TrackStartedEmbed = new MessageEmbed()
          .setAuthor(`En cours de lecture ♪`, this.botconfig.IconURL)
          ...
          if (!cmd.name || !cmd.description || !cmd.run)
            return this.log(
              "Impossible de charger la commande: " +
                file.split(".")[0] +
                ", Raison : Le fichier n'a pas de nom d'exécution/desciption."
            );
          this.commands.set(file.split(".")[0].toLowerCase(), cmd);
          this.log("Commandes chargé: " + file.split(".")[0]);
        });
    });
  }
  ...
  sendError(Channel, Error) {
    let embed = new MessageEmbed()
      ...
      .setFooter(
        "Si vous pensez qu'il s'agit d'un bug, veuillez contactez l'owner !"
      );
  ...
  build() {
    ...
    if (this.botconfig.ExpressServer) {
      this.http.listen(process.env.PORT || this.botconfig.Port, () =>
        this.log("Le serveur Web a été lancé")
      );
    }
  }
