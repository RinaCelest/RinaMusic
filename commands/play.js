...
module.exports = {
  name: "play",
  description: "Aah Jouez vos chansons préférées",
  usage: "[song]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Utilisation - **\`${GuildDB.prefix}play [song]\``
      );
    ...
    try {
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (Searched.loadType === "PLAYLIST_LOADED") {
          ...
          SongAddedEmbed.setAuthor(
            `Liste de lecture ajoutée à la file d'attente`,
            message.author.displayAvatarURL()
          );
          SongAddedEmbed.addField(
            "En file d'attente",
            `\`${Searched.tracks.length}\` musique(s)`,
            false
          );
      ...
      return client.sendTime(
        message.channel,
        "**Jouer de la musique dans le canal vocal - **" + SearchString
      );
    }
  },
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!member.voice.channel)
        return client.sendTime(
          interaction,
          "❌ | **Vous devez être dans le même canal vocal que moi pour utiliser cette commande..**"
        );
      if (
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        try {
          ...
          if (res.loadType === "LOAD_FAILED") {
            ...
            return client.sendError(
              interaction,
              `:x: | **Il y a eu une erreur lors de la recherche**`
            );
          }
        } catch (err) {
          return client.sendError(
            interaction,
            `Il y a eu une erreur lors de la recherche : ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
