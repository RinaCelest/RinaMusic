...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Utilisation - **\`${GuildDB.prefix}search [requête]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Veuillez envoyer le numéro d'identification correct de la chanson"
        );
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "NO_MATCHES":
            ...
            return client.sendTime(
              interaction,
              ":x: | **Aucun résultat n'a été trouvé**"
            );
          case "TRACK_LOADED":
        ...
        try {
          ...
          return client.sendTime(
            interaction,
            `:x: | **Il y a eu une erreur lors de la recherche :** ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
          ...
          case "SEARCH_RESULT":
            ...
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Le nombre que vous avez fourni est supérieur ou inférieur au total de la recherche. Utilisation - \`(1-${max})\``
              );
