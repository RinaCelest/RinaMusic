...
module.exports = {
  name: "grab",
  description: "Enregistre la chanson en cours dans vos DM",
  usage: "",
  permissions: {
    ...
    message.author
      .send(
        new MessageEmbed()
          ...
          .addField(`🔎 Sauvé dans:`, `<#${message.channel.id}>`)
          .setFooter(
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      try {
        let embed = new MessageEmbed()
          .setAuthor(`Chanson sauvée: `, client.user.displayAvatarURL())
          .setThumbnail(
            `https://img.youtube.com/vi/${player.queue.current.identifier}/mqdefault.jpg`
          )
          .setURL(player.queue.current.uri)
          ...
          .addField(`🔎 Sauvegardé dans:`, `<#${interaction.channel_id}>`)
          .setFooter(
            `Demandé par: ${player.queue.current.requester.tag}`,
            player.queue.current.requester.displayAvatarURL({
              dynamic: true,
